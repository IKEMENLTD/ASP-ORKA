...
class SQLDatabase extends SQLDatabaseBase
{
	...
	function __construct($dbName, $tableName, $colName, $colType, $colSize ){
		...
		if( !$this->connect ){
			throw new InternalErrorException("SQLDatabase() : DB CONNECT ERROR. -> mysql_connect( ".$SQL_SERVER." )¥n");
		}
		if(  !$this->connect->select_db( $dbName )  ){
			throw new InternalErrorException("SQLDatabase() : DB CONNECT ERROR. -> mysql_select_db( ". $dbName. " )¥n");
		}
