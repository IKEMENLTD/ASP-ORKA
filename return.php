<?PHP
	...
	/*        for($i=0 ; $row > $i ; $i++ ){
	    ...
	    case 'zenginkyo':
			...
			$head .= '21';//種別コード(総合) 2
			$head .= '0';//コード区分(JIS) 1
			$head .= sprintf('%10s',substr($branch_code,0,2).($bank_type==1?1:2).$number);//委託者コード 10
	...
	//			$data .= sprintf('%15s',mb_convert_kana($ndb->getData( $nrec, 'branch'), "k"));//被仕向支店名15
	        ...
	        $trailer ="";
			$trailer .= '2';//データ区分 1
			$trailer .= sprintf('%6d',$row);//合計件数 6
			$trailer .= sprintf('%12d',$cnt);//合計金額 12
